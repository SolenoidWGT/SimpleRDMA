...
void* client_thread_func(void* arg) {
	...
	for (i = 0; i < num_peers; i++) {
		for (j = 0; j < num_concurr_msgs; j++) {
			ret = post_write_with_imm(msg_size, lkey, (uint64_t)buf_ptr, (uint32_t)i, qp[i], buf_ptr,
			                          ib_res.remote_mr_info[i].rkey, ib_res.remote_mr_info[i].addr);
	...
	while (stop != true) {
		...
		for (i = 0; i < n; i++) {
			...
			if (wc[i].opcode == IBV_WC_RECV) {
				ops_count += 1;
				debug("IBV_WC_RECV ops_count = %ld", ops_count);
