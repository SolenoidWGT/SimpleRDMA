...
int connect_qp_server() {
	...
	for (i = 0; i < num_peers; i++) {
		...
		ret = modify_qp_to_rts(ib_res.qp[peer_ind], remote_qp_info[i].qp_num, remote_qp_info[i].lid);
		check(ret == 0, "Failed to modify qp[%d] to rts", peer_ind);

		log("\tqp[%" PRIu32 "] <-> qp[%" PRIu32 "]", ib_res.qp[peer_ind]->qp_num, remote_qp_info[i].qp_num);
	}
...
int connect_qp_client() {
	...
	log("Remote server mr info addr:%p, length:%ld, rkey: %d", server_MRinfo->addr, server_MRinfo->length,
	    server_MRinfo->rkey);
...
int setup_ib() {
	...
	log("Local server mr info addr:%p, length:%ld, rkey: %d", server_MRinfo->addr, server_MRinfo->length,
	    server_MRinfo->rkey);
