/* Copyright (C) 2003,2004 Andi Kleen, SuSE Labs.
   ...
   libnuma is free_numa software; you can redistribute it and/or
...
static inline void nodemask_set_compat(nodemask_t* mask, int node) {
	mask->n[node / (8 * sizeof(unsigned long))] |= (1UL << (node % (8 * sizeof(unsigned long))));
}
...
static inline int nodemask_isset_compat(const nodemask_t* mask, int node) {
	if ((unsigned)node >= NUMA_NUM_NODES)
		return 0;
	if (mask->n[node / (8 * sizeof(unsigned long))] & (1UL << (node % (8 * sizeof(unsigned long)))))
		return 1;
	return 0;
}
...
/* Return node size and free_numa memory */
...
static inline void* numa_alloc_interleaved_subset_compat(size_t size, const nodemask_t* mask) {
	struct bitmask tmp;
...
static inline void numa_interleave_memory_compat(void* mem, size_t size, const nodemask_t* mask) {
	struct bitmask tmp;
...
static inline void numa_tonodemask_memory_compat(void* mem, size_t size, const nodemask_t* mask) {
	struct bitmask tmp;
...
static inline int numa_sched_getaffinity_compat(pid_t pid, unsigned len, unsigned long* mask) {
	struct bitmask tmp;
...
static inline int numa_sched_setaffinity_compat(pid_t pid, unsigned len, unsigned long* mask) {
	struct bitmask tmp;
